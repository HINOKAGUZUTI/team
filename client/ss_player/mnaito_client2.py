...
class PlayerClient:
    ...
    def initial_move(self):
        largest_block = max(self._player.usable_blocks(), key=lambda b: np.sum(Block(b, BlockRotation(0)).block_map))
        ...
        for rot in rotations:
            ...
            for y in range(target_position.y - np.size(block.block_map, axis=0) + 1, target_position.y + 1):
                for x in range(target_position.x - np.size(block.block_map, axis=1) + 1, target_position.x + 1):
                    try:
                        piece = Board.PaddedBlock(self._board, block, Position(x, y))
                        if self._board.can_place_first_block(self._player, piece):
                            to_s = "0123456789ABCDE"
                            return f"{largest_block.name}{rot.value}{to_s[x]}{to_s[y]}"
                    except ValueError:
                        continue
